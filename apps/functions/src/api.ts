...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!projectId) throw new ApiError('FAILED_PRECONDITION', 'GCLOUD_PROJECT env var not set', 412);

    let credentialsRef: { secretResourceName: string } | undefined;
    if (body.authType === 'API_KEY') {
      if (!body.apiKey) throw new ApiError('INVALID_ARGUMENT', 'apiKey required for API_KEY integrations', 400);
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    const currentVersion = (templateSnap.data()?.currentVersion as number) ?? 0;
...
app.post(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (body.inputSource === 'UPLOAD' && !body.runId) {
      throw new ApiError(
        'INVALID_ARGUMENT',
        'runId is required for UPLOAD runs to ensure deterministic storage paths',
        400
      );
    }
    ...
    if (body.templateVersionId) {
      ...
      version = (versionData?.version as number) ?? 0;
      const versionTemplateId = versionData?.templateId as string | undefined;
    ...
    if (body.inputSource === 'INTEGRATION') {
      ...
      integrationSnapshot = {
        ...
        authType: (data?.authType as any) === 'OAUTH' ? 'OAUTH' : 'API_KEY',
...
app.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const lastOrder = snap.docs.length ? (snap.docs[snap.docs.length - 1]!.data() as any).order : null;
...
app.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const lastIndex = snap.docs.length ? (snap.docs[snap.docs.length - 1]!.data() as any).index : null;
...
app.get(
  '/v1/qc-runs/:runId/chat-results/:chatResultId/rule-results',
  asyncHandler(async (req, res) => {
    const auth = await requireAuth(req.header('authorization'));
    requireRole(auth.role, 'Viewer');
    ...
    if (!runId) throw new ApiError('INVALID_ARGUMENT', 'runId is required', 400);
    if (!chatResultId) throw new ApiError('INVALID_ARGUMENT', 'chatResultId is required', 400);
    ...
    if (!chatSnap.exists) throw new ApiError('NOT_FOUND', 'Chat results not found', 404);
