export type ChatSplitStrategy = 'CHAT_ID' | 'SEPARATORS' | 'SPEAKER_BLOCKS' | 'SINGLE';
...
function rebuildFromLines(lines: string[], startLine: number, endLineExclusive: number): string {
...
function isSeparatorLine(line: string): boolean {
  ...
  if (/^(begin|end)\s+(chat|conversation|transcript)\b/i.test(t)) return true;
  if (/^end\s+of\s+(chat|conversation)\b/i.test(t)) return true;
  return false;
}

function parseLabeledParticipant(line: string): { role: 'operator' | 'customer'; name: string } | null {
  const m = line.match(/^\s*(operator|agent|advisor|rep)\s*:\s*(.+?)\s*$/i);
  if (m) return { role: 'operator', name: m[2]!.trim() };
  const n = line.match(/^\s*(customer|client|caller|buyer)\s*:\s*(.+?)\s*$/i);
  if (n) return { role: 'customer', name: n[2]!.trim() };
  return null;
}

function findChatIdHeader(line: string): { chatId: string } | null {
...
function computeSegmentsFromStarts(lines: string[], startLines: number[]): { segments: { start: number; end: number }[] } {
...
function filterNonTrivialSegments(lines: string[], segments: { start: number; end: number }[]): { start: number; end: number }[] {
  // Avoid producing lots of tiny segments.
  const minChars = 120;
  return segments.filter((s) => rebuildFromLines(lines, s.start, s.end).length >= minChars);
}

export function splitTranscriptIntoChats(text: string): {
  strategy: ChatSplitStrategy;
  warning?: string;
  chats: ChatSegment[];
} {
  ...
  // F) Chat ID headers
  const chatIdStarts: { lineIndex: number; chatId: string }[] = [];
  for (let i = 0; i < lines.length; i++) {
    const id = findChatIdHeader(lines[i]!);
    if (id) chatIdStarts.push({ lineIndex: i, chatId: id.chatId });
  }
  if (chatIdStarts.length >= 2) {
    const { segments } = computeSegmentsFromStarts(
      lines,
      chatIdStarts.map((x) => x.lineIndex)
    );
    const usable = filterNonTrivialSegments(lines, segments);
    if (usable.length >= 2) {
      const chats: ChatSegment[] = usable.map((seg, idx) => {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (j < lines.length) sepStarts.push(j);
  }
  ...
  const participantStarts: { lineIndex: number; participants: { operator?: string; customer?: string } }[] = [];
  for (let i = 0; i < lines.length; i++) {
    const first = parseLabeledParticipant(lines[i]!);
    if (!first) continue;

    const participants: { operator?: string; customer?: string } = {};
    ...
    if (hasBoth) participantStarts.push({ lineIndex: i, participants });
  }

  if (participantStarts.length >= 2) {
    const { segments } = computeSegmentsFromStarts(
      lines,
      participantStarts.map((x) => x.lineIndex)
    );
    const usable = filterNonTrivialSegments(lines, segments);
    if (usable.length >= 2) {
      const chats: ChatSegment[] = usable.map((seg, idx) => {
  ...
  return {
    strategy: 'SINGLE',
    warning:
      'Could not reliably detect multiple chats in this file. The entire upload was treated as a single chat. If this is wrong, export chats with a Chat ID header or separators between chats.',
