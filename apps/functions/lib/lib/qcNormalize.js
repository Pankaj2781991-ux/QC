...
export async function detectInputType(buffer, fileName, contentType) {
    ...
    if (ft?.mime?.startsWith('audio/'))
        return 'audio';
    if (ft?.mime === 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')
        return 'xlsx';
    if (ft?.mime === 'application/json')
        return 'json';
    if (ft?.mime?.startsWith('text/'))
        return 'text';
    return 'unknown';
}
export function normalizeInlineJson(value) {
